...
/**
 * Returns site-wide health checks for the Moodle status report.
 *
 * @return \core\check\check[]
 */
function tiny_authory_tech_get_checks(): array {
    return [new \tiny_authory_tech\check\webservices()];
}
...
/**
 * Returns true if the plugin is globally disabled via the AUTHORY-TECH-INSTANCE-ENABLE env var.
 *
 * @return bool True if disabled, false if enabled
 */
function tiny_authory_tech_is_instance_disabled(): bool {
    $envval = getenv('AUTHORY-TECH-INSTANCE-ENABLE');
    if ($envval !== false) {
        return strtoupper(trim($envval)) !== 'TRUE';
    }
    return (bool) get_config('tiny_authory_tech', 'disabled');
}
...
function tiny_authory_tech_coursemodule_standard_elements($formwrapper, $mform) {
    ...
    $techstatus   = tiny_authory_tech_status($formwrapper->get_current()->course);
    ...
    if (in_array($module, constants::NAMES)) {
        ...
        $mform->setDefault('authory_tech_status', $state);
    }
...
function tiny_authory_tech_upload_multipart_record($filerecord, $filenamewithfullpath, $wstoken, $answertext) {
    ...
    try {
        ...
        $postdata = [
            ...
            'cmid' => $filerecord->cmid,
            'person_id' => $filerecord->userid,
            'class_id' => $filerecord->courseid,
            'ws_token' => $wstoken,
        ...
        if ($result === false) {
            mtrace("  [authory_tech] cURL error for file {$filerecord->id}: " . $curl->error);
            return false;
        }

        if ($httpcode < 200 || $httpcode >= 300) {
            mtrace("  [authory_tech] Upload failed for file {$filerecord->id}: HTTP {$httpcode} — {$result}");
            return false;
        }

        mtrace("  [authory_tech] File {$filerecord->id} uploaded OK (HTTP {$httpcode})");
        return true;
    } catch (moodle_exception $e) {
        mtrace("  [authory_tech] Exception uploading file {$filerecord->id}: " . $e->getMessage());
        if (isset($tempfilepath) && file_exists($tempfilepath)) {
            unlink($tempfilepath);
        }
        return false;
...
/**
 * Publishes a file record to the SQS keystroke queue.
 *
 * @param stdClass $filerecord The file record from tiny_authory_tech_files
 * @param string   $wstoken   Moodle web service token (stored in session for callbacks; may be empty)
 * @param string   $answertext Original submission text
 * @param string   $queueurl  SQS queue URL
 * @param string   $accesskey SQS access key
 * @param string   $secretkey SQS secret key
 * @return bool True on success, false on failure
 */
function tiny_authory_tech_publish_to_sqs($filerecord, $wstoken, $answertext, $queueurl, $accesskey, $secretkey): bool {
    $events = json_decode($filerecord->content, true) ?: [];

    $message = json_encode([
        'institution_id'      => 0,
        'resource_id'         => (string)$filerecord->id,
        'cmid'                => (string)$filerecord->cmid,
        'person_id'           => (string)$filerecord->userid,
        'class_id'            => (string)$filerecord->courseid,
        'ws_token'            => (string)$wstoken,
        'original_submission' => (string)$answertext,
        'events'              => $events,
    ], JSON_UNESCAPED_UNICODE);

    $ok = \tiny_authory_tech\sqs::send_message($queueurl, $accesskey, $secretkey, $message);

    if (!$ok) {
        mtrace("  [authory_tech] SQS publish failed for file {$filerecord->id}");
        return false;
    }

    mtrace("  [authory_tech] File {$filerecord->id} published to queue OK");
    return true;
}

/**
 * Tests connectivity to the ML server by calling /verify-token with the given URL.
 *
...
function authory_tech_test_connection($url) {
    ...
    $token     = get_config('tiny_authory_tech', 'secretkey');
    $remoteurl = rtrim($url, '/') . '/verify-token';
    $moodleurl = $CFG->wwwroot;

    if (empty($token)) {
        return json_encode(['status' => false, 'message' => 'No secret key configured.']);
    }
    ...
    try {
        ...
        if ($result === false) {
            return json_encode(['status' => false, 'message' => $curl->error]);
        }

        $decoded = json_decode($result);
        if ($decoded && isset($decoded->status) && $decoded->status) {
            return json_encode(['status' => true, 'message' => $decoded->message ?? null]);
        }
        return json_encode(['status' => false, 'message' => $result]);
    } catch (moodle_exception $e) {
