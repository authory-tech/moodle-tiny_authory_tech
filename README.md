# Authory.tech Moodle TinyMCE Plugin

At SEPTUM QA, we're focused on the writing process. By capturing key event data (also known by the scary euphemism "key logging"), we can make new opportunities for teaching, learning, and research in a low-cost, low-effort way, all in the existing workflows of your course and site.
...
**Premium/Subscription:** Authory.tech's plugin is designed to interact with our server as a paid service. This integration is optional and adds the following capabilities:
...
If you have questions, comments, or would like to request a trial API key, please reach out to us at contact@authory.tech

## Support

Found a bug or have a feature request? Please report it on our public issue tracker:
https://github.com/authory-tech/moodle-tiny_authory_tech/issues
...
- Unzip the zip file in /path/to/moodle/lib/editor/tiny/plugins/authory_tech folder or upload the zip file in the install plugins options from site administration: Site Administration -> Plugins -> Install Plugins -> Upload zip file

### Install using git clone

Go to Moodle Project `root/lib/editor/tiny/plugins/authory_tech` directory and clone code by using the following commands:

```
git clone https://github.com/authory-tech/moodle-tiny_authory_tech.git authory_tech
```
...
`Site Administration->Plugins->Authory.tech`
...
for subscription please reach out to us at **contact@authory.tech**.
...
3. Global settings for Enabling or Disabling Authory.tech for all courses.

By entering an agreement with Authory.tech, an API URL and key will be provided to manage the premium features. A custom threshold for API-generated values of identify verification is also available to tune the threshold for displaying a green check verification.
...
> **Note:** Users who want to use and get Authory.tech support in the **OU Blog** plugin must install the support plugin. Check for more updates about it at https://authory.tech.

## License

Copyright (C) 2023 Cursive Technology, Inc.
Copyright (C) 2026 SEPTUM QA
...
This plugin is a fork of [moodle-tiny_cursive](https://github.com/cursiveinc/moodle-tiny_cursive) by Cursive Technology, Inc., modified and maintained by SEPTUM QA
