...
function tiny_authory_tech_check_subscriptions() {
    ...
    try {
        ...
        if ($result && !empty($result->status)) {
            ...
            set_config('plan', $plan, 'tiny_authory_tech');

            // Record the first moment a trial plan is activated.
            if ($plan === 'trial') {
                if (!get_config('tiny_authory_tech', 'trial_start')) {
                    set_config('trial_start', time(), 'tiny_authory_tech');
                }
            } else {
                // Clear trial_start when the institution upgrades away from trial.
                set_config('trial_start', '', 'tiny_authory_tech');
            }

            // Persist server-supplied limits so the plugin can enforce them locally.
            // The server may omit these fields; fall back to plan defaults.
            $defaults = ['trial' => [35, 2, 1], 'centro' => [300, null, null], 'institution' => [null, null, null]];
            [$deflimit, $defcourse, $defteacher] = $defaults[$plan] ?? [null, null, null];

            set_config('student_limit', $result->student_limit ?? $deflimit, 'tiny_authory_tech');
            set_config('course_limit', $result->course_limit ?? $defcourse, 'tiny_authory_tech');
            set_config('teacher_limit', $result->teacher_limit ?? $defteacher, 'tiny_authory_tech');
